...
function ContentModal({ purpose, handleClose }) {
    ...
    const handleSave = (e) => {
        if (purpose === 'featured') {
            dispatch(updateChannel({ channelId: channel?._id, featuredVideo: selectedVideo }));
        }
        if (purpose === 'trailer') {
            dispatch(updateChannel({ channelId: channel?._id, channelTrailer: selectedVideo }));
        }
    };
                    ...
                    <div className='relative w-full p-10 max-w-6xl mx-auto bg-gray-900 rounded-md shadow-lg space-y-5'>
                        <div className='bg-gray-900'>
                            <div className='flex justify-between items-center'>
                                <p className='text-gray-200 font-bold text-2xl'>Select a video</p>
                                <div className='flex gap-3'>
                                    <button
                                        ...
                                        className={`px-10 group relative flex items-center justify-center rounded-sm border border-transparent ${
                                            !selectedVideo ? `bg-red-800` : `bg-red-700`
